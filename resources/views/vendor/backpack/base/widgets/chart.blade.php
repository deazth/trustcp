@php
  ...
  if(isset($widget['param'])){
    // use custom if widget called with param
    $controllerName = $widget['controller'];
    $r = new ReflectionClass($controllerName);
    // $p1 = $widget['param']['p1'] ?? null;
    // $p2 = $widget['param']['p2'] ?? null;
    // $p3 = $widget['param']['p3'] ?? null;

    $controller = $r->newInstanceArgs($widget['param']);
  } else {
    // use default
