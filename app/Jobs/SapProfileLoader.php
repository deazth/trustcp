...
class SapProfileLoader implements ShouldQueue
{
      ...
      foreach ($omdata as $key => $value) {
        ...
        try {
          ...
          if($user){
            ...
            if(strlen($trimmedstaffno) > 0 && $trimmedstaffno != '0'){
              // find using staff no.
              ...
              $value->load_status = 'R';
              $value->save();
              \Illuminate\Support\Facades\Log::error('SapProfileLoader [' . $trimmedstaffno . '] Rejected at ' . gethostname());
          ...
          $bossuser = User::where('persno', $value->reportingto_no)->first();
          if($bossuser){
            $user->report_to_id = $bossuser->id;
          } else {
            $user->report_to_id = null;
          }
          $user->status = 1;
          $user->lob_descr = $value->lob_descr;
          ...
          $value->load_status = 'E';
          $value->save();
          \Illuminate\Support\Facades\Log::error('SapProfileLoader Error at ' . gethostname() . ' ' . $e->getMessage());
        }
