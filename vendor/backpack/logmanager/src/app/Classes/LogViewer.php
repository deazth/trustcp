...
class LogViewer
{
    ...
    {
      // dd($file);
        $logsPath = storage_path('app/reports/logs');
    ...
    {
        $files = glob(storage_path().'/app/reports/logs/*.log');
        ...
        if ($basename && is_array($files)) {
            foreach ($files as $k => $file) {
                ...
                if ($disk->exists('storage/app/reports/logs/'.$file_name)) {
                    $files[$k] = [
                        'file_name'     => $file_name,
                        'file_size'     => $disk->size('storage/app/reports/logs/'.$file_name),
                        'last_modified' => $disk->lastModified('storage/app/reports/logs/'.$file_name),
                    ];
                }
            }
